...
export const useStores = () => {
  ...
  // Update a store
  const updateStore = useCallback(async (id: string, storeData: StoreUpdateRequest) => {
    try {
      setLoading(true);
  ...
  const getStoreById = useCallback(async (id: string) => {
    try {
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to fetch store';
      setError(errorMessage);
      showError(errorMessage);
      throw err;
    } finally {
